...
//		System.out.println("result is "+ dateFormat.format(myDate));
	...
	public static String randomDOB(String age) {
		LocalDate date = LocalDate.now().plusYears(-Integer.parseInt(age));
		int randomMonth = RandomUtils.getRandomNumberBetween(date.getMonthValue(), 1);
		int daysInMonth = date.withMonth(randomMonth).lengthOfMonth();
		int randomDay = RandomUtils.getRandomNumberBetween(daysInMonth, 1);
		return date.withMonth(randomMonth).withDayOfMonth(randomDay).format(DateTimeFormatter.ofPattern("dd/MMM/yyyy"))
				.replaceFirst("^0+(?!$)", "");
	}
